...
Most product code lives in `app/`, the Expo/React Native application and its own Git working tree. Use `app/app/` for Expo Router routes, `app/features/` for domain modules such as `partner` and `progress`, `app/store/` for Zustand stores, `app/components/` for shared UI, and `app/lib/` or `app/hooks/` for utilities. Static assets live in `app/assets/`. Root files are mainly planning docs plus SQL changes in `database/migrations/`.

## Build, Test, and Development Commands
Run app commands from `app/` unless noted otherwise.

- `cd app && npm install` installs dependencies.
- `cd app && npm start` starts the Expo dev server.
- `cd app && npm run android` launches the Android target.
- `cd app && npm run ios` launches the iOS target.
- `cd app && npm run web` launches the web target.
- `cd app && npm test -- --runInBand` runs the Jest suite once.
- `docker compose up expo-app` runs the app in the root Docker workflow with Node 20.

## Coding Style & Naming Conventions
TypeScript runs in `strict` mode via `app/tsconfig.json`. Match the existing style: 2-space indentation, single quotes, and semicolons. Prefer functional components and hooks. Use PascalCase for component exports, `useX` for hooks, `*-store.ts` for Zustand stores, and kebab-case filenames under `app/features/`. Use the `@/` alias for app-local imports when it improves readability.

## Testing Guidelines
Jest is configured with `jest-expo` and `@testing-library/react-native` in `app/jest.config.js`. Place tests in `app/__tests__/` or alongside legacy component tests under `app/components/__tests__/`. Name files `*.test.ts`, `*.test.tsx`, or `*-test.js` to match the current pattern. Mock platform services such as AsyncStorage and Supabase in unit tests, and reset Zustand-backed state in `beforeEach`. No coverage threshold is enforced, so add or update tests for each changed store, hook, or route flow.

## Commit & Pull Request Guidelines
The nested `app/` repository is present, but the current branch has no commits yet, so there is no historical message convention to copy. Use short imperative commit subjects such as `Add partner invite expiry test`. Pull requests should summarize behavior changes, list commands run, link related issues or planning docs, and include screenshots for UI changes.

## Security & Configuration Tips
Start from `app/.env.example` and keep real `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` values out of version control. Do not commit local Expo state from `app/.expo/`. Keep database changes additive and numbered in `database/migrations/` such as `004_create_partner_tables.sql`.
